...
def owd_doc_pair2(doc1, doc2, w2v,
                  lambda1, lambda2, sigma, max_iter, metric):
    snts1 = sent_tokenize(doc1)
    snts2 = sent_tokenize(doc2)
    len = min(len(snts1), len(snts2))
    distances = []
    for i in range(len):
        distances.append(owd_snt_pair(snts1[i], snts2[i],
                                      w2v, lambda1, lambda2,
                                      sigma, max_iter, metric))
...
if __name__ == "__main__":
    # load word vectors
    fw2v = "../../../../data/raw/Tencent-w2v/w2vgood_20170209.model"
    w2v = load_word2vec(fw2v, 200)
    metric = "euclidean"
    d = owd_snt_pair("", "", w2v, 50, .1, 1, 200, metric)
    print d
    d = owd_snt_pair("", "总统 在 演说", w2v, 50, .1, 1, 200, metric)
    print d
    d = owd_doc_pair("奥巴马 在 白宫 发表 演讲", "总统 在 演说", w2v, 50, .1, 1, 200, 50, .1, 1, 200, metric)
    print d
    metric = "cosine"
    d = owd_snt_pair("奥巴马 在 白宫 发表 演讲", "总统 在 演说", w2v, 50, .1, 1, 200, metric)
    print d
    d = owd_snt_pair("", "总统 在 演说", w2v, 50, .1, 1, 200, metric)
    print d
    d = owd_doc_pair("奥巴马 在 白宫 发表 演讲", "总统 在 演说", w2v, 50, .1, 1, 200, 50, .1, 1, 200, metric)
